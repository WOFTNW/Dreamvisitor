...
public class CmdSchedule implements DVCommand {
                ...
                .withSubcommands(
                        new CommandAPICommand("interval")
                                ...
                                .withArguments(
                                        ...
                                        new IntegerArgument("interval-minutes", 1),
                                        new GreedyStringArgument("command")
                                )
                                .executesNative((sender, args) -> {
                                    String name = (String) args.get("name");
                                    assert name != null;
                                    Integer intervalMinutes = (Integer) args.get("interval-minutes");
                                    assert intervalMinutes != null;
                                    String commandInput = (String) args.get("command");
                                    assert commandInput != null;

                                    List<String> commands = parseCommands(commandInput);
                        ...
                        new CommandAPICommand("daily")
                                ...
                                .withArguments(
                                        new StringArgument("name"),
                                        new TextArgument("time").includeSuggestions(
                        ...
                        new CommandAPICommand("cron")
                                ...
                                .withArguments(
                                        new StringArgument("name"),
                                        new TextArgument("pattern").includeSuggestions(
                                ...
                                .withArguments(
                                        ...
                                        .replaceSuggestions(
                                                ArgumentSuggestions.strings(info -> CommandScheduler.getInstance().getSchedules().stream()
                                                        .map(Schedule::getName)
                                                        .toArray(String[]::new))),
                                        new IntegerArgument("index", 1)
                                )
                                .executesNative((sender, args) -> {
                                    String name = (String) args.get("name");
                                    assert name != null;
                                    Integer index = (Integer) args.get("index");
                                    assert index != null;
                                ...
                                .withArguments(
                                        ...
                                        .replaceSuggestions(
                                                ArgumentSuggestions.strings(info -> CommandScheduler.getInstance().getSchedules().stream()
                                                        ...
                                                        .toArray(String[]::new))),
                                        new IntegerArgument("index", 1),
                                        new IntegerArgument("delay-ticks", 1)
                                )
                                .executesNative((sender, args) -> {
                                    String name = (String) args.get("name");
                                    assert name != null;
    ...
    private static @NotNull String getTypeInfo(@NotNull Schedule schedule) {
        String typeInfo;

        switch (schedule.getType()) {
            case INTERVAL ->
                    typeInfo = "every " + ChatColor.GREEN + schedule.getIntervalMinutes()
                            + ChatColor.WHITE + " minutes";
            case DAILY ->
                    typeInfo = "daily at " + ChatColor.GREEN + schedule.getDailyTime();
            case CRON ->
    ...
    private static @NotNull List<String> parseCommands(@NotNull String commandInput) {
